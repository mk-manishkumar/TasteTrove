...
export const createRestaurantController = async (req, res) => {
  try {
    ...
    res.status(201).send({ success: true, message: "New Restaurant Created successfully" });
  } catch (error) {
    console.log(error);
    res.status(500).send({ success: false, message: "Error In Create Restaurant api", error });
  }
};
...
export const getAllRestaurantController = async (req, res) => {
  try {
    ...
    console.log(error);
    res.status(500).send({ success: false, message: "Error In Get ALL Restaurant API", error });
  }
};
...
export const getRestaurantByIdController = async (req, res) => {
  try {
    ...
    if (!restaurantId) {
      return res.status(404).send({ success: false, message: "Please Provide Restaurant ID" });
    }
    ...
    console.log(error);
    res.status(500).send({ success: false, message: "Error In Get Restaurant by id api", error });
  }
};
...
export const deleteRestaurantController = async (req, res) => {
  try {
    const restaurantId = req.params.id;
    if (!restaurantId) {
      return res.status(404).send({ success: false, message: "No Restaurant Found OR Provide Restaurant ID" });
    }
    await Restaurant.findByIdAndDelete(restaurantId);
    res.status(200).send({ success: true, message: "Restaurant Deleted Successfully" });
  } catch (error) {
    console.log(error);
    res.status(500).send({ success: false, message: "Error in delete restaurant api", error });
  }
};
